...
public class SO_MoveToPoint : SO_Enemy_Substate
{
    ...
    [SerializeField] private bool _isBulletHellPhaseAnimation;
    [ShowIf(nameof(_isBulletHellPhaseAnimation), true)][SerializeField] EnemyBulletHellAnimation bulletHellanimation;
    [ShowIf(nameof(_isBulletHellPhaseAnimation), false)][SerializeField] EnemyStates enemyStates = EnemyStates.Moving;
    ...
    public override IEnumerator Execute(Enemy enemy)
    {
        ...
        float speed;
        float distance = Vector3.Distance(enemy.gameObject.transform.position, targetPos);
        if (_toPlayer)
        {
            while (distance >= _distance)
            {
                ...
                if (!_constantSpeed)
                {
                    speed = _speedCurve.Evaluate(_time / _timeToReachPoint);
                    targetPos = new Vector3(_playerInfo.position.x, enemy.gameObject.transform.position.y, enemy.gameObject.transform.position.z);
                    enemy.gameObject.transform.position = Vector3.MoveTowards(originPos, targetPos, speed);
                    distance = Vector3.Distance(enemy.gameObject.transform.position, targetPos);
                }
                else
                {
                    speed = _moveSpeed  * Time.deltaTime;
                    originPos = enemy.gameObject.transform.position + _offset;
                    targetPos = new Vector3(_playerInfo.position.x, enemy.gameObject.transform.position.y, enemy.gameObject.transform.position.z);
                    enemy.gameObject.transform.position = Vector3.MoveTowards(originPos, targetPos, speed);
                    distance = Vector3.Distance(enemy.gameObject.transform.position, targetPos);
                }
            ...
            if (Observer.OverallState is BulletHellGameplayState) enemy.GravitySet(0);
            else enemy.GravitySet(50);
            yield break;
        }
    ...
    {
        if (_isBulletHellPhaseAnimation) return;
        if (!_toPlayer)
        {
            if (enemy.Waypoints[_waypointIndex].position.x < enemy.transform.position.x)
            {
    ...
    {
        if (_isBulletHellPhaseAnimation)
        {
            return AnimationHash.Enemy_BulletHellPhase_Animation((int)bulletHellanimation);
        }
