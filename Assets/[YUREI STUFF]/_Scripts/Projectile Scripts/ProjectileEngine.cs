...
public class ProjectileEngine : MonoBehaviour
{
    ...
    void BulletPattern()
    {
        ...
        for (int i = 0; i < _projectiles.Count; i++)
        {
            ...
            x = Physics2D.OverlapCircle((Vector3)_projectilePos[i], _data.hitboxSize, LayerMask.GetMask("Player")); //This Creates GC Alloc
            if (x != null)
            {
                _setActiveInfo[i] = false;
                _hitPlayer[i] = true;
    ...
    void DivideAngle()
    {
        ...
        {
            if (_distanceToPlayer[i] > _collissionDistance) continue;
            Gizmos.DrawWireSphere((Vector3)_projectilePos[i], _data.hitboxSize);
        }
    }
}
